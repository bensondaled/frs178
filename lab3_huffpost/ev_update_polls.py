...
def process_polls():
    ...
    for day in campaign_season():
        ...
        for state in sorted(state_names, key=lambda x: state_names[x]):
            ...
            cleaned_polls = drop_overlapping_polls(polls_ended_before_day, state)
	...
	print "STATE", state
        ...
        while (not stop):
            ...
            if len(xmldoc.childNodes[0].childNodes) > 1:
                max_filenum += 1
                f = open("%s%s%s.xml" % (archive_dir, state, str(page_num)), 'w')
                s=xmldoc.toxml(encoding="utf-8")
                f.write(s)
                f.close()
                print "Wrote %d polls to %s" % (0.5 * (len(xmldoc.childNodes[0].childNodes) - 1), "%s%s%s.xml" % (archive_dir, state, str(page_num)))
...
def subpop_parse(subpop):
    ...
    pop = int(get_opt_subelem(subpop, "observations", "1")) # if no population size, pretend it's 1
...
def process_subpop(poll_org, method, state, start_date, end_date, subpop, question_state):
    global state_file, national_file
    ...
    f.write("%s,\"%s\",%d,\"%s\",\"%s\"," % (question_state, poll_org, pop, vtype, method))
...
def process_pollfile(xmldoc, state):
    ...
    for q in questions:
        try:
            ...
            poll_state = q.getElementsByTagName("state")[0].childNodes[0].nodeValue
            ...
            if len(subpops) >= 2:
                for subpop in subpops:
                    ...
                    if vtype == "Likely Voter":
                        process_subpop(poll_org, method, state, start_date, end_date, subpop, poll_state)
            else:
                for subpop in subpops:
                    process_subpop(poll_org, method, state, start_date, end_date, subpop, poll_state)
...
def store_prev_outcome():
    ...
    prev_outcome["MA"] = (30, datetime.date(2016,1,1),
        datetime.date(2016,1,1), datetime.date(2016,1,1), 1, "Election 2012")
    prev_outcome["MS"] = (-13, datetime.date(2016,1,1),
        datetime.date(2016,1,1), datetime.date(2016,1,1), 1, "Election 2012")
    prev_outcome["MT"] = (-14, datetime.date(2016,1,1),
        datetime.date(2016,1,1), datetime.date(2016,1,1), 1, "Election 2012")
    prev_outcome["NE"] = (-20.5, datetime.date(2016,1,1),
        datetime.date(2016,1,1), datetime.date(2016,1,1), 1, "Election 2012")
